...
internal sealed class MainForm : Form
{
    ...
    public MainForm(string[] extraArgs)
    {
        ...
        _spinButton = new Button { Text = "Reverse Spin", AutoSize = true, Enabled = false };
        _spinButton.Click += OnReverseSpinClicked;
        ...
        buttonStrip.Controls.Add(_spinButton);
    ...
    private void OnReverseSpinClicked(object? sender, EventArgs e)
    {
        // The UI thread is the pump thread, so game state is safe to touch from event handlers,
        // including nodes typed via the game assembly's script classes.
        foreach (var cube in _engine!.Tree.CurrentScene
                     .GetNode<Node3D>("Flair/BlueCubes").GetChildren().OfType<SpinningCube>())
            cube.SpinSpeed = -cube.SpinSpeed;
    }
